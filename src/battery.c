...
static void run_charging_callbacks(struct k_work *work)
{
    ...
    printk("Battery: Charger %s\n", is_charging ? "connected" : "disconnected");
...
static void sample_periodic_handler(struct k_work *work)
{
    ...
    if (ret)
    {
        printk("Battery: Failed to get voltage\n");
        goto reschedule;
    }
...
int battery_register_charging_callback(battery_charging_callback_t callback)
{
    ...
    {
        printk("Battery: Max callbacks reached\n");
        return -ENOMEM;
    }
...
int battery_get_millivolt(uint16_t *battery_millivolt)
{
    ...
    // Voltage divider circuit (calibrated values)
    const uint16_t R1 = 1037; // 1M ohm (calibrated)
    ...
    if (ret)
    {
        printk("Battery: ADC read failed (error %d)\n", ret);
    }
...
int battery_get_percentage(uint8_t *battery_percentage, uint16_t battery_millivolt)
{
    ...
    for (uint16_t i = 0; i < BATTERY_STATES_COUNT - 1; i++)
    {
        ...
        if (battery_millivolt <= voltage_high && battery_millivolt >= voltage_low)
        {
            ...
            int32_t voltage_diff = battery_millivolt - voltage_low;
...
int battery_start_sampling(uint32_t interval_ms)
{
    if (interval_ms == 0)
    {
        printk("Battery: Interval must be > 0\n");
        return -EINVAL;
    }
    ...
    printk("Battery: Sampling every %d ms\n", interval_ms);
    return 0;
}
...
int battery_stop_sampling(void)
{
    ...
    printk("Battery: Stopped sampling\n");
    return 0;
}
...
int battery_init()
{
    ...
    if (ret)
    {
        printk("Battery: ADC setup failed (error %d)\n", ret);
        return ret;
    }
    ...
    {
        printk("Battery: GPIO charging_enable not found\n");
        return -EIO;
    }

    if (!gpio_is_ready_dt(&read_enable))
    {
        printk("Battery: GPIO read_enable not found\n");
        return -EIO;
    }

    if (!gpio_is_ready_dt(&charge_speed))
    {
        printk("Battery: GPIO charge_speed not found\n");
        return -EIO;
    }
    ...
    if (ret)
    {
        printk("Battery: Failed to configure charging_enable (error %d)\n", ret);
        return ret;
    }
    ...
    if (ret)
    {
        printk("Battery: Failed to configure interrupt (error %d)\n", ret);
        return ret;
    }
    ...
    if (ret)
    {
        printk("Battery: Failed to configure read_enable (error %d)\n", ret);
        return ret;
    }
    ...
    if (ret)
    {
        printk("Battery: Failed to configure charge_speed (error %d)\n", ret);
        return ret;
    }
    ...
    // Set fast charging and enable reading
    ret |= battery_set_fast_charge();
    if (ret)
    {
        printk("Battery: Failed to set fast charging (error %d)\n", ret);
        return ret;
    }
    ...
    if (ret)
    {
        printk("Battery: Failed to enable reading (error %d)\n", ret);
        return ret;
    }
